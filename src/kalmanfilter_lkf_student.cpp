...
constexpr double INIT_POS_STD = 5;
constexpr double INIT_VEL_STD = 5;
...
void KalmanFilter::predictionStep(double dt)
{
    if (!isInitialised() && INIT_ON_FIRST_PREDICTION)
    {
        ...
        state << 0, 0, 5.0 * std::cos(M_PI / 4), 5.0 * std::sin(M_PI / 4);
...
void KalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
    if (isInitialised())
    {
        ...
        auto innovation = Z - H * state;

        auto innovation_cov = H * cov * H.transpose() + R;
