...
void KalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap &map)
{
    if (isInitialised())
    {
        ...
        if (meas.id != -1 && map_beacon.id != -1)
        {
            ...
            Matrix2d innovation_cov = Matrix2d::Zero();
            ...
            double delta_x = state(0) - map_beacon.x;
            double delta_y = state(1) - map_beacon.y;
            double z_hat_r = std::sqrt(delta_x * delta_x + delta_y * delta_y);
            double z_hat_q = wrapAngle(std::atan2(-delta_y, -delta_x) - state(2));
            z_hat << z_hat_r, z_hat_q;

            innovation = z - z_hat;
            innovation(1) = wrapAngle(innovation(1));

            H << delta_x / z_hat_r, delta_y / z_hat_r, 0, 0,
                -delta_y / z_hat_r / z_hat_r, delta_x / z_hat_r / z_hat_r, -1, 0;

            innovation_cov = H * cov * H.transpose() + R;

            auto kalman_gain = cov * H.transpose() * innovation_cov.inverse();

            state += kalman_gain * innovation;
            cov = (Matrix4d::Identity() - kalman_gain * H) * cov;
        }
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        F << 1, 0, -dt * state(3) * sin(state(2)), dt * cos(state(2)),
